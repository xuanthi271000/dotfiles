...
create_command_group {ces} -info "IC Compiler II - useful procedures"

proc ces_help {} {
	echo "\n"
	echo "ces: IC Compiler II - useful aliases"
	uplevel {
		foreach n [lsort [array names al]] {
			echo [format " %-40s%s" $n $al($n)]
			alias $n $al($n)
		}
	}
	echo "\n"
...
define_proc_attributes _run_stage \
	-info "run and log a single stage of an optimization command" \
	-command_group ces \
	-define_args {
		{command "command to run - one of place_opt, refine_opt, clock_opt" command string required}
		{stage "stage to run - e.g. initial_place, initial_drc, ..." stage string required}
		{-logfile "name of log file - default is place_opt.<stage>.log" "" string optional}
	}
# _run_stage place_opt initial_place
# _run_stage place_opt initial_drc
# _run_stage place_opt initial_opto
# _run_stage place_opt final_place
# _run_stage place_opt final_opto
...
set VIEW_COMMAND "/home/thip/tools/ICC2/view.tk"
...
proc vman { args } {
  if { $::in_gui_session } {
    gui_show_man_page $args
  } else {
    v man $args
  }
}
define_proc_attributes vman \
  -info "If GUI has been started, show man page using GUI, else using view" \
	-define_args { 
		{args "Command or app option or ..." args} 
	} \
	-command_group ces
